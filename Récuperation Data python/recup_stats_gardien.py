...
def get_fbref_goalkeeper_stats(lst_league_names, lst_seasons):
        ...
        for league_name_long in league_names_long:
            ...
            if not os.path.exists(os.path.join(data_dir_fbref + f'/raw/goalkeeper/fbref_goalkeeper_stats_combined_latest.csv')):
                ...
                if(saison_name_short!=""):
                  ...
                  url_keepers_adv = f'https://widgets.sports-reference.com/wg.fcgi?css=1&site=fb&url=%2Fen%2Fcomps%2F{league_name_short}%2F{saison_name_short}%2Fkeepersadv%2Fplayers%2F{season}-{league_name_long}&div=div_stats_keeper_adv'
                ...
                # Remove accents
                df_all['Player'] = (df_all['Player'].str.encode('latin-1', errors='ignore').str.decode('UTF-8'))
                df_all['Squad'] = (df_all['Squad'].str.encode('latin-1', errors='ignore').str.decode('UTF-8'))
    
                #Update of name columns 
                df_all = df_all.rename({'Save%.1': 'Save%Penalty','Att':'Att_launched','Att.1':'Att_passes','Att.2':'Att_goal_kicks','Launch%.1':'Launch_goal_kicks','AvgLen.1':'AvgLen_goal_kicks',
                'Gls.1':'Gls/90','Ast.1':'Ast/90','G-PK.1':'G-PK/90','xG.1':'xG/90','xA.1':'xA/90','npxG.1':'npxG/90','npxG+xA.1':'npxG+xA/90','On-Off.1':'On-Off_xG'}, axis=1)            
    ...
    all_files = glob.glob(os.path.join(data_dir_fbref + f'/raw/goalkeeper/*/*/fbref_goalkeeper_stats_*_latest.csv'))
...
lst_league_names = ['Premier-League', 'Ligue-1', 'Serie-A','Bundesliga', 'La-Liga','Champions-League','Europa-League']    #'Big-5-European-Leagues','Premier-League', 'Ligue-1', 'Bundesliga', 'Serie-A', 'La-Liga', 'Major-League-Soccer']
